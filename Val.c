...
int TVal_isarray(TValue *v){
	return TVal_type(v) == TVAL_ARRAY;
}
...
static inline void array_dcotr(void *_)
{
	TVal_array *a = cast(TVal_array*,_);
	size_t i;
	for(i=0;i < a->nsize;++i){
		TValue *v = a->array[i];
		if(v) TVal_release(v);
...
{
	TVal_array *array = malloc(sizeof(*array));
	array->rcount  = 1;
	array->cap     = INIT_SIZE;
	array->nsize   = 0;
...
static int array_resize(TVal_array *a,size_t newcap)
{
	TValue **array = realloc(a->array,newcap*sizeof(*array));	
	if(!array) return -1;
	size_t c;
	if(array != a->array)
		a->array = array;
	for(c = a->cap; c < newcap; ++c)
		a->array[c] = NULL;
	a->cap = newcap;
	return 0;
}
...
TValue     *TVal_array_get(TValue  *a,size_t i)
{
	assert(a->tt == TVAL_ARRAY);
	TValue     *v = NULL;
	if(a->tt == TVAL_ARRAY){
		TVal_array *array = cast(TVal_array*,a);
		if(i > 0 && i <= array->nsize)
			v = array->array[i-1];
	}
	return TVal_retain(v);
}

int 		TVal_array_set(TValue  *a,size_t i,TValue *val)
{
	assert(a->tt == TVAL_ARRAY);
	if(a->tt == TVAL_ARRAY){
		TVal_array *array = cast(TVal_array*,a);
		...
		if(v && val){
			TVal_release(v);
			array->array[i-1] = TVal_retain(val);	
		}else if(v && !val){
			//remove
			return TVal_array_remove(a,i);	
		}
		return 0;
	}
	return -1;
...
#define MAX_ARRAY_SZIE 0x80000000
...
int        TVal_array_push(TValue *a,TValue *v)
{
	...
	if(a->tt == TVAL_ARRAY){
		TVal_array *array = cast(TVal_array*,a);
		if(array->nsize == array->cap){
			if(!(array->cap < MAX_ARRAY_SZIE && 
				 !array_resize(array,array->cap << 1)))
				return -1;
		}
		array->array[array->nsize++] = TVal_retain(v);	
		return 0;
	}
	return -1;
}

int        TVal_array_remove(TValue  *a,size_t i)
{
	assert(a->tt == TVAL_ARRAY);
	if(a->tt == TVAL_ARRAY){
		TVal_array *array = cast(TVal_array*,a);
		if(i > 0 && i <= array->nsize)
		{
			TValue *v = array->array[i-1];
			if(v){
				TVal_release(v);
				//re arrange
				size_t c = i-1;
				for(; c < array->nsize-1;++c)
					array->array[c] = array->array[c+1];
				array->array[array->nsize-1] = NULL;
				--array->nsize;
				if(array->cap > 16 && array->nsize < array->cap >> 1)
					array_resize(array,array->cap >> 1);
				return 0;
			}
		}
...
size_t      TVal_array_len(TValue *a)
{
	assert(a->tt == TVAL_ARRAY);
	if(a->tt == TVAL_ARRAY)
		return cast(TVal_array*,a)->nsize;
	return 0;
}

static iter  TVal_array_next(TValue *_,iter *it)
{
	TVal_array *a = cast(TVal_array*,_);	
	iter ret;
	size_t i = 0;
	...
	ret.key = TVal_New_integer(i+1);	
	ret.value = TVal_retain(v);
	ret.i     = i;
	return ret;	
}
...
{
	if(t->tt == TVAL_ARRAY)
		return TVal_array_next(t,it);
	else if(t->tt == TVAL_TABLE)
...
static TValue *toArray(lua_State *L,int idx,lua_Integer len)
{
...
static void pushArray(lua_State *L,TValue *v)
{
	size_t n = TVal_array_len(v);
	size_t i = 1;
	lua_newtable(L);	
	for(; i <= n; ++i){
		TValue *_v = TVal_array_get(v,i);
		pushVal(L,_v);
		lua_rawseti(L,-2,i);
		TVal_release(_v);
	}
...
void TVal_print(TValue *v){
	switch(TVal_type(v)){
		...
		case TVAL_ARRAY:{
			printf("array:%lx",cast(Val_Integer,v));
			break;
		}		
		default:{
